...
- **AC-0200** Given a completed local recovery, when its TS extraction has classified the eval sites, then the register's preflight findings for that repo are replaced by the textual scan reconciled with those claims (AC-0099), so every surface quotes the post-recovery classification and no pending wording survives.
