...
- **AC-0207** Given any T0 adapter (TS/JS, Python, Go, Java, Kotlin, Terraform) whose edges name an endpoint no parsed declaration defines, when it closes over that endpoint, then the placeholder node is never left without provenance: it cites the edge that named it (evidence span, the producing extractor), states a human-readable `reason`, and records its `boundary` (ADR-0031). It is Confirmed only when the adapter proves the boundary — an **external** import the repository provably cannot provide (a JVM package that no Java or Kotlin source declares, lies beneath, or encloses; in Go module mode a standard-library path or a module outside every repository module and local `replace`; a Python top-level name no repository directory or module shares; a bare JS/TS specifier matching no tsconfig alias or `baseUrl` path, workspace package, `#` subpath import, or repository source directory, citing the `package.json` that declares it when one does) or an **internal** repository-declared package (a JVM package, a Go package directory, a workspace package cited by its manifest). Every other endpoint — an in-system import with no unique declaration, an unresolved relative import, symbol, or resource, or any import when the adapter cannot decide — stays an explicit Gap carrying that reason, so external dependencies never count as system gaps. JVM imports of a type the repository declares exactly once resolve to the declaring File; a relative JS/TS import of a non-source asset (stylesheet, image, JSON) names that file rather than a `.ts`-guessed phantom and is internal when the file exists; an observed (T1) state resolution supersedes a Terraform placeholder's Gap provenance; and the Gap register classes placeholder gaps by their reason rather than an internal edge label.
