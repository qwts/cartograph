...
- **AC-0201** Given a TS/JS `eval()` or `new Function()` whose code argument is const-shaped (an identifier or member expression) but not provable to a literal, when the file is extracted, then the site becomes exactly one explicit `Gap` node — `attempted_tiers: ["T0"]`, a reason stating the argument could not be proven to a literal, and Gap-tier provenance citing the call site plus every unproven argument — owned through `DEPENDS_ON` by its enclosing symbol (or its File at top level), with an id keyed by repo, path and site offset so the same source yields the same Gap on every run and in the captured production lane; nothing about the site is asserted as Confirmed, and proven or dynamic sites emit no such Gap.
