...
#[test]
fn eval_coverage_progress_reports_every_file_and_stops_without_a_partial_list() {
    // AC-0197 (#235): the slow preflight walk announces each file before
    // parsing it (for live progress), and a break stops it before the next
    // parse without returning a partial claim list.
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join("a.ts"), "eval(\"function a() {}\");\n").unwrap();
    std::fs::write(dir.path().join("b.ts"), "eval(\"function b() {}\");\n").unwrap();
    std::fs::write(dir.path().join("c.ts"), "export const c = 1;\n").unwrap();

    let mut seen = Vec::new();
    let sites = eval_coverage_with_progress(dir.path(), &id(), &mut |step| {
        seen.push((step.path.to_string(), step.done, step.total));
        std::ops::ControlFlow::Continue(())
    })
    .unwrap()
    .expect("an uninterrupted walk completes");
    assert_eq!(
        seen,
        vec![
            ("a.ts".to_string(), 0, 3),
    ...
    assert_eq!(sites, eval_coverage(dir.path(), &id()).unwrap());
    assert_eq!(sites.len(), 2);

    let mut announced = 0;
    let stopped = eval_coverage_with_progress(dir.path(), &id(), &mut |step| {
        announced += 1;
        if step.done == 1 {
    ...
    assert_eq!(announced, 2, "no file is announced after the break");
}
