...
- **AC-0201** Given independent reads of an investigation's detail and saved result, when they disagree about whether a result exists (a read that straddles the result commit), then the observation is rejected as inconsistent: the task is never recorded as completed without findings or as unfinished with findings, previously saved findings and activity stay visible, and the next refresh reads a consistent pair.
...
- **Trace:** H4 · `agents`, `llm`, `core-graph`, `context-hub`, `app`, `ui` · — · T-0180..0191,T-0201
