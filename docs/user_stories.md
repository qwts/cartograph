...
- **Priority:** Must · **Status:** In-Progress
- **AC-0180** Given a scoped question and a selected specialist, when an investigation starts, then the host binds the exact versioned Domain analyst or Evidence auditor definition, provider, normalized scope and limit profile to a distinct investigation contract; all findings remain T3/InferredWeak and no synthetic edge, target write or confirmed fact is created.
