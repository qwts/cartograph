...
- **AC-0203** Given an extraction that cites one (source, destination, label) relation from several sites, when it is ingested, then the stored edge keeps the props, tier and confidence of the last occurrence in deterministic extraction order and cites the evidence spans of every occurrence with that same tier, confidence and extractor — sorted, de-duplicated and bounded, with any spans beyond the bound counted on the edge rather than dropped silently — and whenever more than one occurrence is unioned its content hash is derived from their distinct fact hashes together with the canonical span list (spans beyond the bound and their count included), so adding, removing or moving any cited site changes the hash, a single-site relation keeps its extracted hash, and re-ingesting the same commit yields an identical edge and graph hash.
