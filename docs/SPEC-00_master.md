...
- **Determinism check:** re-ingesting the same commit under the same source registration must yield an identical graph (content-hash equality) — a CI invariant. Determinism is scoped per registration (#430). Managed GitHub clones use a stable `owner/name` repository key, so their facts do not depend on where the clone lives. Direct local sources get a random per-registration key (`local/src_<hex>`, ADR-0023, SPEC-05) that appears in fact ids and evidence. A local tree registered twice, registered as a local source on another installation, or relocated to a new local checkout under a new registration therefore hashes differently, and no cross-installation or relocated-local-checkout equality is claimed for local sources.
