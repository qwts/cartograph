...
- **AC-0200** Given a completed local recovery, when its TS extraction has classified the eval sites, then the register's preflight findings for that repo are replaced by the textual scan of the captured bytes the claims were proven on (not the live tree), reconciled with those claims (AC-0099), so every surface quotes the post-recovery classification and no pending wording survives; a preflight of that repo that began before the reconciliation cannot overwrite it afterwards.
