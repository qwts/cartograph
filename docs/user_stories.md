...
- **AC-0144** Given a registered repository and revision label, when Repo facts and system display are produced, then canonical fact properties/hash omit operational roots and mutable display metadata, while an operational DTO supplies human labels; repeated recovery for a retained registration is stable without claiming cross-installation graph equality.
