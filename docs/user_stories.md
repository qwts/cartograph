...
- **AC-0198** Given a running preflight, when I cancel it (or start another preflight, which supersedes it, or switch the source away from a local tree), then it stops before parsing the next file, persists no findings, reports that it was cancelled rather than a raw error, and a superseded result never replaces the current one.
