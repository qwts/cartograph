...
- **AC-0195** Given an extraction in which a node id or a (source, destination, label) relation occurs more than once — one relation cited from several call or import sites, or distinct declarations sharing one id — when it is ingested (direct, add-repo, or add-system), then the graph keeps exactly one fact per node id and per relation, the summary's node and edge totals are the distinct facts the whole operation published — the union across every repo it loaded (Repo nodes included) and every later whole-graph stage (observed BACKS stitching, found-ADR relinking), less any fact those stages retracted — so a first ingest into an empty graph quotes the same totals as the Workspace graph counts, each per-layer row counts distinct facts, and the collapsed occurrences, including a globally keyed fact published by more than one repo, are reported explicitly as merged nodes, merged edges, and node identity collisions (one id carrying differing facts) — never silently.
...
- **AC-0203** Given an extraction that cites one (source, destination, label) relation from several sites, when it is ingested, then the stored edge keeps the props, tier and confidence of the last occurrence in deterministic extraction order and cites the evidence spans of every occurrence with that same tier, confidence and extractor — sorted, de-duplicated and bounded, with any spans beyond the bound counted on the edge rather than dropped silently — and whenever more than one occurrence is unioned its content hash is derived from their distinct fact hashes together with the canonical span list (spans beyond the bound and their count included), so adding, removing or moving any cited site changes the hash, a single-site relation keeps its extracted hash; a collapsed relation publishes without a primary-source receipt binding, since no single receipt attested it; and re-ingesting the same commit yields an identical edge and graph hash.
...
- **Trace:** M0–M3 · `ingest`, `core-graph`, `app`, `ui` · — · T-0001..0003,T-0049..0050,T-0076..0078,T-0085,T-0094,T-0195..0198,T-0202..0203
...
- **AC-0098** Given a Kotlin repository (`.kt`/`.kts`), when it is ingested, then classes, interfaces, objects, data classes, enums, and functions (top-level, member, and extension) become Confirmed T0 Symbols with exact evidence spans; same-scope and import-proven cross-file calls (type/object receivers and imported top-level functions) are joined deterministically repo-wide, with a declared-package import whose target cannot be proven — or is declared ambiguously more than once — failing closed to an explicit Gap, calls on foreign packages and unproven receivers asserting nothing (a foreign import itself closes over a proven external boundary, AC-0207); import-proven Spring Web annotations compose class-level `@RequestMapping` with `@{Get,Post,Put,Delete,Patch}Mapping` into Confirmed Endpoint/HANDLES facts (lookalikes without the proving import produce no endpoints, and a present-but-non-literal path — on the method or the class base — fails closed to an explicit route Gap instead of a Confirmed endpoint); Preflight/Settings report Kotlin as installed (`t0.adapter-kotlin`) rather than a requestable planned adapter; and the ingest summary reports Kotlin separately.
- **AC-0099** Given a TS/JS source with an `eval()` or `new Function()` whose code argument is compile-time-known — a string literal, a substitution-free template, or a same-file `const`/const-object member proven by binding (never a name coincidence; for `new Function` the last string argument is the body and earlier ones are parameter names), then the string content is parsed by the same extractor and its Symbols, CALLS, and event sites are emitted as Confirmed T0 facts carrying `via: "eval"` with evidence citing the argument's span at the eval site, the enclosing symbol gains a CALLS edge into the extracted code's entry symbol so flows cross the eval boundary, and recovery reconciles Preflight's `inline-eval` findings from that same extraction (adapter-supplied claims, so the textual scan and the AST proof never disagree; AC-0199/AC-0200): a fully proven site's finding closes, while a const-shaped-but-unproven argument downgrades to an explicit potential Gap and any other dynamic argument — including a shadowed local `eval`/`Function` binding, which is not the global and yields no facts — stays an Unsupported finding, never a guessed fact.
- **AC-0100** Given how Node/TS actually resolve modules, when the directory-wide reconciliation runs, then (a) an extensionless relative import matching no file resolves through the `index.<ext>` directory convention, (b) an explicit `.js`/`.jsx` relative specifier with no such file on disk resolves to its `.ts`/`.tsx` sibling (the NodeNext idiom), (c) a bare specifier matching a tsconfig/jsconfig `paths` alias or explicit `baseUrl` resolves to the real file with the deciding config cited as provenance evidence, and (d) a bare specifier naming a workspace package inside the ingested tree resolves through that package's `exports`/`module`/`main` map with its `package.json` cited; every resolution is a proof against files that exist — unmatched specifiers keep their explicit `mod:` node or placeholder, and external packages are never resolved into. The tsconfig parse is shared with the toolchain detector's stored facts (parse once, `adapters-fw::tsconfig`).
- **AC-0192** Given a proven Spring mapping in Java, when its path is a string literal or an array initializer of string literals (`@GetMapping({ "/vets" })`, `value = { "/a", "/b" }`), then each literal becomes its own Confirmed Endpoint composed with the class base, and an empty or absent path takes Spring's default; when the path on the method or the class-level `@RequestMapping` is present but not provably literal (a constant, a concatenation, any expression), then the mapping yields an explicit route Gap bound to its handler and no Endpoint — never a fabricated `/`.
- **AC-0199** Given a local preflight, when it scans TS/JS sources, then it performs no TS extraction (no AST parse, #243) and reports every textual `eval(`/`new Function(` line as an Unsupported `inline-eval` finding marked pending AST proof at recovery — none is claimed closed or downgraded, and none is dropped.
- **AC-0200** Given a completed local recovery, when its TS extraction has classified the eval sites, then the register's preflight findings for that repo are replaced by the textual scan of the captured bytes the claims were proven on (not the live tree), reconciled with those claims (AC-0099), so the register and the Preflight surface (which receives the reconciled report, potential-Gap lane included, with the recovery's summary) quote the post-recovery classification and no pending wording survives. The reconciliation happens only after the recovered facts are published, so a recovery that fails earlier leaves the pending findings in place; a preflight of that repo that began before the reconciliation cannot overwrite it afterwards, while a preflight started after recovery is newer and reports the lines as pending again until the next recovery (attested reuse of the recovery's proof is #438).
- **AC-0201** Given a TS/JS `eval()` or `new Function()` whose code argument is const-shaped (an identifier or member expression) but not provable to a literal, when the file is extracted, then the site becomes exactly one explicit `Gap` node — `attempted_tiers: ["T0"]`, a reason stating the argument could not be proven to a literal, and Gap-tier provenance citing the call site plus every unproven argument — owned through `DEPENDS_ON` by its enclosing symbol (or its File at top level), with an id keyed by repo, path and site offset so the same source yields the same Gap on every run and in the captured production lane; nothing about the site is asserted as Confirmed, and proven or dynamic sites emit no such Gap; a proven `eval()` whose code itself holds such a site is claimed const-unproven rather than covered, so Preflight keeps its potential-Gap finding, and a nested Gap's content hash derives from its final namespaced id.
...
- **AC-0207** Given any T0 adapter (TS/JS, Python, Go, Java, Kotlin, Terraform) whose edges name an endpoint no parsed declaration defines, when it closes over that endpoint, then the placeholder node is never left without provenance: it cites the edge that named it (evidence span, the producing extractor), states a human-readable `reason`, and records its `boundary` (ADR-0031). It is Confirmed only when the adapter proves the boundary — an **external** import the repository provably cannot provide (a JVM package that no Java or Kotlin source declares, lies beneath, or encloses, with every header readable — multiline file annotations included; in Go module mode a standard-library path or a module outside every repository module and local `replace`; a Python top-level name no repository directory or module shares; a bare JS/TS specifier matching no tsconfig alias or `baseUrl` path, workspace package, `#` subpath import, or repository source directory, citing the `package.json` that declares it when one does; under a tsconfig that `extends` another, whose inherited aliases are not loaded, only a manifest-declared dependency) or an **internal** repository-declared package (a JVM package, a Go package directory, a workspace package cited by its manifest). Every other endpoint — an in-system import with no unique declaration, an unresolved relative import, symbol, or resource, or any import when the adapter cannot decide — stays an explicit Gap carrying that reason, so external dependencies never count as system gaps. a placeholder named by several references is Confirmed only when every reference proves the same boundary (one unresolved reference keeps it a Gap), including across adapters in a polyglot ingest (one adapter's Gap is never overwritten by another adapter's Confirmed placeholder for the same id); a JVM header scan reads Unicode annotation names and arguments without panicking, and fails closed to unknown on anything it cannot parse; JVM imports whose complete target (a type, a top-level function, or a member whose Symbol the type defines) the repository declares exactly once resolve to the declaring File, and a prefix alone never does; a relative JS/TS import of a non-source asset (stylesheet, image, JSON) names that file rather than a `.ts`-guessed phantom and is internal when the file exists and the import does not climb above the repository root; an observed (T1) state resolution supersedes a Terraform placeholder's Gap provenance; and the Gap register classes placeholder gaps by their reason rather than an internal edge label.
- **Trace:** M1,M10 · `adapters-lang-ts`, `adapters-lang-python`, `adapters-lang-go`, `adapters-lang-java`, `adapters-lang-kotlin`, `adapters-fw`, `core-graph`, `core-prov`, `iac`, `dynamic`, `ingest`, `app`, `ui` · — · T-0004..0006,T-0053..0054,T-0079..0080,T-0095,T-0098..0100,T-0192,T-0199..0200,T-0207
...
- **AC-0057** Given a completed recovery, when the Workspace landing renders, then the outcome tally (open findings split into gaps, unsupported patterns, and no-evidence) and every provenance-health count derive from the single register summary and atlas projection all surfaces share — findings are listed explicitly, never guessed or double-counted — and the outcome badge states the weakest confidence tier actually present among recovered facts (InferredWeak before InferredStrong before Confirmed); open gaps mark the recovery partial via the outcome title and tally but never inflate the badge to an inferred tier no fact carries.
...
- **Actor:** Engine
- **As a** engineer **I want** re-ingesting the same commit under the same source registration to yield an identical graph **so that** outputs are reproducible.
- **Priority:** Must · **Status:** Done
- **AC-0039** Given the same commit and inputs, when re-ingested, then the ordered T0 node/edge identity plus content-hash snapshot is identical and unchanged graph facts are not rewritten; every ingest or evidence path is canonicalized through one shared helper that never stores a platform-specific verbatim prefix (the Windows `\\?\` extended-length form), so the stored path form for the same tree does not vary by platform.
...
- **AC-0060** Given any completed ingest, when it finishes, then a history record persists its tier tallies (counted with the register's own provenance definition), unsupported/no-evidence counts, per-extractor coverage (files in scope, distinct files with facts — a covering adapter with zero facts is a 0% row, never a missing row), and an order-independent whole-graph content hash, so re-ingesting the same commit under the same source registration shows identical hashes in queryable history — determinism observable as data, not only asserted in tests.
- **AC-0205** Given a tree whose own `.gitignore` files exclude paths (for example `build/`, `out/`, `vendor/`), when it is preflighted or recovered through any lane (ordinary, captured, or managed checkout), then every extractor, plugin routing, toolchain detection, and Preflight skip the ignored paths and read the same sorted file set as each other. Git's precedence applies: the innermost `.gitignore` decides and `!` re-includes. Non-ignored files are still read, and the fixed skip lists remain the fallback. Only `.gitignore` files inside the tree count, never ancestor, `.git/info/exclude`, or global excludes. Symlinks are never followed. Env-file indexing alone disregards `.gitignore` (ADR-0030).
- **AC-0208** Given any tree and any "Ingest parallelism" setting (Auto or a fixed 1…N workers in Settings, persisted across restarts), when TS/JS, Python, Go, Java, and Kotlin sources are extracted, then files are parsed on that many workers but merged in sorted walk order. The recovered facts, layer counts, and graph hash set are byte-identical to a serial run, and a failure reports the first failing file in walk order. Auto resolves to max(1, non-efficiency cores − 1), capped at one worker per 2 GiB of physical memory where the platform reports it (macOS, Linux; Windows is uncapped, tracked in #472). Workers run at most a small window ahead of the in-order merge, so peak memory does not grow with the worker count.
- **Security:** —
- **Performance:** Delta re-ingest scales with change size, not repo size. Per-file extraction scales with worker count (#236).
- **Trace:** M10 · `source-walk`, `adapters-lang-*`, `iac`, `ingest`, `core-graph`, `core-prov`, `app`, `ui` · — · T-0039..0040,T-0060,T-0205,T-0208
...
- **AC-0120** Given nested or shadowed callable declarations, when their calls are extracted, then the nearest actual callable owns the calls and direct-call targets require lexical binding proof; unrelated file-wide name matches and unknown member dispatch do not become confirmed links; nested Nest decorators require their actual unshadowed imported bindings.
- **AC-0121** Given unchanged source and existing unambiguous top-level/class callables, when re-extracted, then identities and content hashes are deterministic and existing identities remain stable; anonymous route symbol and endpoint ownership continue to agree, and nested Nest controllers bind HANDLES to the actual qualified decorated-method symbol.
...
- **AC-0123** Given unresolved calls, mutation, loops or exception control flow, when rule evidence is recovered, then unsupported behavioral interpretation and dependencies remain explicit gaps rather than omitted prerequisites or invented semantics; parser-recovered exits or controlling branches yield an explicit unsupported_rule_syntax omission Gap rather than Confirmed rule evidence or a silently dropped throw.
...
- **AC-0125** Given recovered rule evidence, when the inventory, shared context and exports are inspected, then owner, conditions, local effect, dependencies, confidence and incomplete interpretation are consistent; computed callable keys disclose omitted source names and unresolved runtime keys; the pinned benchmark records only fully supported matches.
...
### US-0027 — Keep registered sources distinct and resolve evidence on the host
- **Actor:** Engineer and context consumer
- **As a** engineer **I want** lasting source identities and exact host-owned source lookup **so that** similarly named projects never share recovered ownership or display another project's code.
- **Priority:** Must · **Status:** In-Progress
- **AC-0140** Given separate local roots or linked worktrees, when registered and reopened through independent connections, then the same canonical root retains its opaque identity and separate roots remain distinct even with equal basenames or Git metadata; missing/moved/replaced roots follow the explicit logical-registration semantics in SPEC-05 without a physical-continuity claim.
- **AC-0141** Given normalized managed origins including same-basename local mirrors, when a clone is added or replaced, then the host reserves separate owned destinations before cloning, cleans only its own failed attempt, rejects unowned or substituted destinations, and preserves the existing authentication/no-partial-clone contract.
- **AC-0142** Given registered sources, when preflight/gate re-scan, direct ingest, manifest-local or clone intake runs, then adapters, caches, findings, reconciliation and metrics use one registered repository key; re-ingesting one source preserves another's facts and root-specific plugin permissions do not transfer.
- **AC-0143** Given a selected evidence reference, when the UI or agent task reader requests source, then the host resolves the exact registered repository and preserves bounded rooted reads; unknown, unavailable or legacy-unmapped references return unavailable without a first-repository or caller-root fallback and without claiming revision freshness.
- **AC-0144** Given a registered repository and revision label, when Repo facts and system display are produced, then canonical fact properties/hash omit operational roots and mutable display metadata, while an operational DTO supplies human labels and equal names remain disambiguated by exact repository identity; repeated recovery for a retained registration is stable without claiming cross-installation graph equality.
- **AC-0145** Given a new source-bound ingest job or a legacy path-only job, when retry is requested, then the new job resolves its registered source and the legacy job requires explicit fresh ingestion before any retry mutation; original job kind/history remains unchanged.
- **AC-0146** Given legacy graph/findings and durable history, when identity migration or later graph/job cleanup runs, then disposable legacy graph ownership is rebuilt and generated findings retire once, while registered sources and historical jobs, metrics, decisions and staged proposal JSON/IDs/reviews remain unchanged.
- **AC-0147** Given concurrent managed-source operations or an owning process that exits, when guards are acquired, then shared readers exclude replacement, exclusive guards last through parser/enrichment use and publication, contention fails without lock cycles, and OS handle closure releases ownership without making an interrupted source ready; guards are acquired before graph/cache locks and nested work reuses them. ADR snapshot validation and its fact patch share one SQLite write transaction, rejecting stale input without writes and rolling back failed patches.
...
### US-0031 — Retain the exact source basis of agent investigations
- **Actor:** Engineer reviewing agent evidence
- **As a** engineer **I want** agent tasks and proposals to retain their actual source selections **so that** I can distinguish captured parser input, unverified evidence and later changes without altering what was reviewed.
- **Priority:** Must · **Status:** In-Progress
- **AC-0172** Given a supported gap task, when evidence is planned and acquired, then typed supporting facts and original citation occurrences follow deterministic candidate order under separate lookahead, attempted-read, supplied-item and captured-validation-byte limits, with explicit omissions and unread tails; unreadable legacy candidates do not consume supplied slots, and unused-tail failures cannot abort a completed prefix.
- **AC-0173** Given graph facts and current source associations, when a task is prepared, then one coherent read selects the bounded window, sorted per-source/access-kind guards are acquired outside store mutexes and one authoritative recheck rejects a changed visited basis; equal facts with different receipts remain distinct, and missing, invalid and absent bindings never collapse into one state.
- **AC-0174** Given a participating citation, when its evidence is acquired, then exact source/fact/digest/receipt/inventory/range checks return original retained strict UTF-8 bytes after checkout mutation or disappearance; corrupt, forgotten, unsupported or over-budget captured reads fail without live fallback, while valid absence remains explicitly working-tree unverified.
- **AC-0175** Given a prepared task and result, when staged, then immutable v2 identity binds graph and selected associations, every supplied evidence origin/text fingerprint and all candidate memberships/summary fingerprints; literal v1 bytes/IDs/reviews remain unchanged, malformed version/shape/bounds fail and raw task text remains transient under the existing replay policy.
- **AC-0176** Given preview or single/class execution, when evidence origins or receipts change, then the same prepared path changes the exact redacted payload and consent identity even for equal span text, preserving per-tier consent, broker limits and one-instance cloud grants without provider or source-write authority from retention.
- **AC-0177** Given a selected staged proposal, when current-basis assessment is requested, then separate graph, association, retained-availability and unverified dimensions report the observed state or operational failure without rewriting history, persisting a blanket fresh flag or certifying legacy references; source retention previews count actual v2 receipt references.
- **AC-0178** Given reingest, forgetting, cancellation or cleanup during a model call, when a result completes, then valid retained execution stages the original copied task basis, releases acquisition guards before the call and preserves completed results; later assessment reports changes without altering graph, provenance, exports or legacy history.
- **AC-0179** Given preview and staged-history UI, when mixed evidence, selection limits or a changed/unavailable basis is shown, then the user can distinguish each origin and assessment dimension, late responses cannot replace another proposal selection and accepted results remain awaiting reconciliation.
- **Security:** Source acquisition is host-qualified and local; no arbitrary roots, target writes, captured-to-live fallback, extra egress, raw task archive or confidence upgrade.
- **Performance:** 64 request lookahead/attempt cap plus one slot key, 8 candidates/12 supplied spans, 8 KiB per span/48 KiB combined, 128 MiB captured validation work, 64 KiB source basis and 128 KiB staged record; graph/manifest costs remain separate.
- **Trace:** H3 prerequisite #401 (SPEC-09) · agents, core-graph, app, ui · — · T-0172..0179


### US-0032 — Investigate recovered context with durable specialists
- **Actor:** Developer
- **As a** developer **I want** named specialists to query recovered context, inspect original evidence and retain cited findings **so that** I can investigate business behavior, design evidence and gaps from the app and revisit the exact basis of an answer.
- **Priority:** Must · **Status:** In-Progress
- **AC-0180** Given a scoped question and a selected specialist, when an investigation starts, then the host binds the exact versioned Domain analyst or Evidence auditor definition, provider, normalized scope and limit profile to a distinct investigation contract; all findings remain T3/InferredWeak and no synthetic edge, target write or confirmed fact is created. New @2 defaults explain that empty initial input requires discovery through host-executed query/read actions; the original @1 definitions remain readable and reproducible without changing saved task meaning.
- **AC-0181** Given concurrent identical starts or a lost start acknowledgment, when the same origin/request nonce is submitted, then one transaction returns the same durable task and initial Job; changed material conflicts, execution attachment is atomic, and history survives job cleanup.
- **AC-0182** Given a prepared frozen graph and cumulative selected-fact ledger, when a specialist queries or acquires additional evidence, then bounded queries use the original authorized snapshot and one coherent union observation rejects graph or previously selected association changes without rebinding inputs or substituting a current revision. Page budgets include evidence inventories; a fitting ordered prefix preserves its continuation cursor, only returned facts enter the ledger, and an individually oversized fact fails without being skipped.
- **AC-0183** Given an admitted typed fact and original receipt role/index, when evidence is read, then retained strict original bytes and nested v2 occurrences remain available after checkout deletion under exact receipt validation; missing, forgotten or invalid participating evidence never falls back, legacy absence remains unverified, and repeated attempts/full-file validation consume cumulative limits.
- **AC-0184** Given a provider action, when the coordinator processes it, then only one complete strict query_context, read_evidence or finish value is admitted, tool actions are journaled before execution, invented references and raw-source replay from any supplied item fail, and invalid output causes no hidden repair or additional model call.
- **AC-0185** Given a local or cloud investigation call, when its bounded provider API executes, then request/response entity-body acquisition, generation reservations, actual stop conditions, verified TLS, no redirects/retries and the remaining task deadline are enforced before action decoding; measured usage and unknown outcomes remain distinct from reservations or estimates and legacy completion remains compatible. Bounded transport @2 requests Ollama JSON actions with optional thinking disabled, binds that version into consent and new provider records, and leaves older unknown transport identities unchanged.
- **AC-0186** Given successive cloud steps, when consent is previewed or approved, then each exact provider/model/payload/step/limit binding requires its own eligible grant consumed with invocation start; stale or reused approval, cancellation, local-only policy and declined consent cause no new call, and hiding the dialog or declining does not authorize fallback.
- **AC-0187** Given cancellation, disconnect, cleanup or process death, when investigation lifecycle is observed, then live execution ownership remains authoritative, a pending invocation is durably distinguishable from pre-call interruption, unknown outcomes are never automatically replayed, and a valid late finish is retained against its original input without executing any late tool action.
- **AC-0188** Given admitted context and findings, when journal/history/result records are stored or loaded, then strict versioned bounded SQL decoding, global admission limits, redacted history, source-free events and transactionally indexed actual receipt references preserve privacy and identity; capacity or storage failures never acknowledge a successful answer or silently evict history.
- **AC-0189** Given an old investigation citation, when it is inspected after reingest or forgetting, then the host reads only the saved receipt occurrence and reports retained availability separately from historical meaning; graph-only and legacy citations disclose their limitations and cannot silently open current source.
- **AC-0190** Given the app investigation surface, when a developer asks, navigates, approves a step, cancels or reconnects, then Workspace, palette and Jobs reach the same durable task, ordered real activity and cited findings; stale responses cannot replace another selection, missing event pages recover explicitly, existing shortcuts remain stable and unknown outcomes have no generic Retry/Resume.
- **AC-0191** Given pinned captured TypeScript inputs and an independent expected answer, when the implementation is verified, then production-module scripted and controlled real-provider investigations record actual query/read/finish steps, citations, role/model/input identities and limits without supplying the oracle; this evidence preserves the full unexecuted H4/H5 cross-ingress obligations and never establishes complete confirmed Vendure rules from T3 output.
- **AC-0204** Given independent reads of an investigation's detail and saved result, when they disagree about whether a result exists (a read that straddles the result commit), then the observation is rejected as inconsistent: the task is never recorded as completed without findings or as unfinished with findings, previously saved findings and activity stay visible, and the next refresh reads a consistent pair.
- **Security:** Per-tier and per-step cloud consent, host-owned scope/execution, closed read tools, all-input replay admission, transient raw payloads and bounded durable history; no target writes or delegation.
- **Performance:** SPEC-10 limits model/tool steps, cumulative context/source/file validation, body acquisition, time, events and total admission; start returns durable identity before expensive work on a worker.
- **Trace:** H4 · `agents`, `llm`, `core-graph`, `context-hub`, `app`, `ui` · — · T-0180..0191,T-0204
