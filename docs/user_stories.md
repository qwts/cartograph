...
- **AC-0195** Given an extraction in which a node id or a (source, destination, label) relation occurs more than once — one relation cited from several call or import sites, or distinct declarations sharing one id — when it is ingested (direct, add-repo, or add-system), then the graph keeps exactly one fact per node id and per relation, the summary's node and edge totals are the distinct facts the load published (the Repo node included), so a first ingest into an empty graph quotes the same totals as the Workspace graph counts, each per-layer row counts distinct facts, and the collapsed occurrences are reported explicitly as merged nodes, merged edges, and node identity collisions (one id carrying differing facts) — never silently.
