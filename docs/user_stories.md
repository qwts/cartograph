...
- **AC-0205** Given a proven Spring mapping in Java or Kotlin, when the class-level `@RequestMapping` base and the method mapping are composed, then segments meet at exactly one `/` (never `//`) and a trailing slash spelled in either literal is kept — `"/api/"` with a bare `@GetMapping` is `/api/` and `"/api"` with `@GetMapping("/")` is `/api/` — so `/a` and `/a/` are distinct Confirmed Endpoints, matching Spring 6's default of no trailing-slash matching; a client fetch path that differs from a route only by a trailing slash never confirms against it.
