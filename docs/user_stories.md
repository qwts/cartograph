...
- **AC-0209** Given a gap register larger than the Gaps lane's grouping threshold (12 rows), when the spec is compiled, then `gap_register.md` groups the gaps into the lane's cause classes (stop reason × extractor, largest first, deterministic ties), details at most 50 classes with up to 5 representative instances each (inline provenance for those instances), and states every omission as a counted line ("N more instances", "N more classes") so its size stays bounded regardless of register size; a `gap_register.json` sidecar lists every instance id by class, every instance still carries its full provenance in the bundle's structured assertions, and nothing is silently dropped.
- **Security:** —
- **Performance:** —
- **Trace:** M9–M10 · `spec`, UI · — · T-0032..0035,T-0057..0058,T-0067,T-0209
